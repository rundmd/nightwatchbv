define([
  ...
  'require'
], function (assert, utils, properties, elements, searchFilters, contentTags, loginPage, actions, pollUntil, require) {
  function contentTagsFilterPage(remote) {
    this.remote = remote;
    //var loginPage;
  }

  contentTagsFilterPage.prototype = {
    constructor: contentTagsFilterPage,
    ...
    multiFilters: function () {
      var session = this.remote;
      var filters = [searchFilters.tag1Filter, allFilters.tag2Filter];
      return session
        .get(properties.BETA_TAB)
        .setFindTimeout(30000)
        .then ( () => {
        ...
        .then( () => {
          return actions.getText(session, elements.RESULTS_COUNT_LOCATOR, 'css')
            .then( (results) => {
              assert.equal(results, properties.MULTI_CONTENT_TAGS_FILTER_RESULTS);
            });
        });
    }, 
    ...
    clearMultiFilters: function () {
      ...
      return session
        ...
        .setFindTimeout(30000)
        .then( () => {
          return this.setFilter(session, searchFilters.contentTagsFilters);
        })
        .sleep(2000)
        //.then( () => {
    ...
    clearAllFilters: function () {
      ...
      return session
        ...
        .setFindTimeout(30000)
        .then( () => {
          return this.setFilter(session, contentTags.filters);
        })
        .sleep(2000)
        .then( () => {
          return actions.clickButton(session, elements.CLEAR_ALL_TAGS_LOCATOR, 'css');
        })
        .sleep(2000) // wait for results to update based on the applied filter
        .then( () => {
          return actions.getText(session, elements.RESULTS_COUNT_LOCATOR, 'css')
