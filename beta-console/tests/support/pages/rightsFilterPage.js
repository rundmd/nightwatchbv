define([
  ...
  function rightsFilterPage(remote) {
    this.remote = remote;
    //var loginPage;
  }

  rightsFilterPage.prototype = {
    constructor: rightsFilterPage,
    ...
    multiOptionFilters: function () {
      ...
      return session
        ...
        .then( () => {
          return actions.getText(session, elements.RESULTS_COUNT_LOCATOR, 'css')
            .then( (results) => {
              assert.equal(results, properties.MULTI_RIGHTS_FILTER_RESULTS);
            });
        });
    }, 
    ...
    allFilters: function () {
      ...
      return session
        ...
        .setFindTimeout(30000)
        .then ( () => {
          return this.setFilter(session, rights.filters);
        })
        .sleep(2000) // wait for results to update based on the applied filter
        .then( () => {
          return actions.getText(session, elements.RESULTS_COUNT_LOCATOR, 'css')
    ...
    clearMultiFilters: function () {
      ...
      return session
        ...
        .setFindTimeout(30000)
        .then( () => {
          return this.setFilter(session, searchFilters.rightsFilters);
        })
        .sleep(2000)
        //.then( () => {
    ...
    clearAllFilters: function () {
      ...
      return session
        ...
        .sleep(2000)
        .then( () => {
          return actions.clickButton(session, elements.CLEAR_ALL_RIGHTS_LOCATOR, 'css');
        })
        .sleep(2000) // wait for results to update based on the applied filter
        .then( () => {
          return actions.getText(session, elements.RESULTS_COUNT_LOCATOR, 'css')
