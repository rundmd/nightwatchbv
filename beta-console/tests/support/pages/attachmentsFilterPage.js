define([
  ...
  'require'
], function (assert, utils, properties, elements, searchFilters, attachments, loginPage, actions, pollUntil, require) {
  function attachmentsFilterPage(remote) {
    this.remote = remote;
    //var loginPage;
  }

  attachmentsFilterPage.prototype = {
    constructor: attachmentsFilterPage,
    ...
    multiOptionFilters: function () {
      var session = this.remote;
      var filters = [searchFilters.photoFilter, allFilters.videoFilter];
      return session
        .get(properties.BETA_TAB)
        .setFindTimeout(30000)
        .then ( () => {
        ...
        .then( () => {
          return actions.getText(session, elements.RESULTS_COUNT_LOCATOR, 'css')
            .then( (results) => {
              assert.equal(results, properties.MULTI_ATTACHMENTS_FILTER_RESULTS);
            });
        });
    }, 
    ...
    clearMultiFilters: function () {
      ...
      return session
        ...
        .setFindTimeout(30000)
        .then( () => {
          return this.setFilter(session, searchFilters.attachmentsFilters);
        })
        .sleep(2000)
        //.then( () => {
    ...
    clearAllFilters: function () {
      ...
      return session
        ...
        .setFindTimeout(30000)
        .then( () => {
          return this.setFilter(session, attachments.filters);
        })
        .sleep(2000)
        .then( () => {
          return actions.clickButton(session, elements.CLEAR_ALL_ATTACHMENTS_LOCATOR, 'css');
        })
        .sleep(2000) // wait for results to update based on the applied filter
        .then( () => {
          return actions.getText(session, elements.RESULTS_COUNT_LOCATOR, 'css')
