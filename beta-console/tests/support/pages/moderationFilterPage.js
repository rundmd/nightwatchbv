define([
  ...
  'require'
], function (assert, utils, properties, elements, searchFilters, moderation, loginPage, actions, pollUntil, require) {
  function moderationFilterPage(remote) {
    this.remote = remote;
    //var loginPage;
  }

  moderationFilterPage.prototype = {
    constructor: moderationFilterPage,
    ...
    multiOptionFilters: function () {
      var session = this.remote;
      var filters = [searchFilters.modQueuedFilter, allFilters.modPendingFilter];
      return session
        .get(properties.BETA_TAB)
        .setFindTimeout(30000)
        .then ( () => {
        ...
        .then( () => {
          return actions.getText(session, elements.RESULTS_COUNT_LOCATOR, 'css')
            .then( (results) => {
              assert.equal(results, properties.MULTI_MOD_FILTER_RESULTS);
            });
        });
    }, 
    ...
    clearMultiFilters: function () {
      ...
      return session
        ...
        .setFindTimeout(30000)
        .then( () => {
          return this.setFilter(session, searchFilters.moderationFilters);
        })
        .sleep(2000)
        //.then( () => {
    ...
    clearAllFilters: function () {
      ...
      return session
        ...
        .setFindTimeout(30000)
        .then( () => {
          return this.setFilter(session, moderation.filters);
        })
        .sleep(2000)
        .then( () => {
          return actions.clickButton(session, elements.CLEAR_ALL_MODERATION_LOCATOR, 'css');
        })
        .sleep(2000) // wait for results to update based on the applied filter
        .then( () => {
          return actions.getText(session, elements.RESULTS_COUNT_LOCATOR, 'css')
