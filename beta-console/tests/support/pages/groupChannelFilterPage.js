define([
  ...
  'require'
], function (assert, utils, properties, elements, searchFilters, channels, loginPage, actions, pollUntil, require) {
  function groupChannelFilterPage(remote) {
    this.remote = remote;
    //var loginPage;
  }

  groupChannelFilterPage.prototype = {
    constructor: groupChannelFilterPage,
    ...
    multiOptionFilters: function () {
      var session = this.remote;
      var filters = [searchFilters.facebookFilter, allFilters.googleplusFilter];
      return session
        .get(properties.BETA_TAB)
        .setFindTimeout(30000)
        .then ( () => {
    ...
    clearMultiFilters: function () {
      ...
      return session
        ...
        .setFindTimeout(30000)
        .then( () => {
          return this.setFilter(session, searchFilters.groupChannelFilters);
        })
        .sleep(2000)
        //.then( () => {
    ...
    clearAllFilters: function () {
      ...
      return session
        ...
        .setFindTimeout(30000)
        .then( () => {
          return this.setFilter(session, channels.filters);
        })
        .sleep(2000)
        .then( () => {
          return actions.clickButton(session, elements.CLEAR_ALL_CHANNELS_LOCATOR, 'css');
        })
        .sleep(2000) // wait for results to update based on the applied filter
        .then( () => {
          return actions.getText(session, elements.RESULTS_COUNT_LOCATOR, 'css')
