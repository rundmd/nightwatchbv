define([
  ...
  'require'
], function (assert, utils, properties, elements, searchFilters, tagging, loginPage, actions, pollUntil, require) {
  function taggingFilterPage(remote) {
    this.remote = remote;
    //var loginPage;
  }

  taggingFilterPage.prototype = {
    constructor: taggingFilterPage,
    ...
    multiFilters: function () {
      var session = this.remote;
      var filters = [searchFilters.taggingQueuedFilter, allFilters.taggingPendingFilter];
      return session
        .get(properties.BETA_TAB)
        .setFindTimeout(30000)
        .then ( () => {
        ...
        .then( () => {
          return actions.getText(session, elements.RESULTS_COUNT_LOCATOR, 'css')
            .then( (results) => {
              assert.equal(results, properties.MULTI_TAGGING_FILTER_RESULTS);
            });
        });
    }, 
    ...
    clearMultiFilters: function () {
      ...
      return session
        ...
        .setFindTimeout(30000)
        .then( () => {
          return this.setFilter(session, searchFilters.taggingFilters);
        })
        .sleep(2000)
        //.then( () => {
    ...
    clearAllFilters: function () {
      ...
      return session
        ...
        .setFindTimeout(30000)
        .then( () => {
          return this.setFilter(session, tagging.filters);
        })
        .sleep(2000)
        .then( () => {
          return actions.clickButton(session, elements.CLEAR_ALL_TAGGING_LOCATOR, 'css');
        })
        .sleep(2000) // wait for results to update based on the applied filter
        .then( () => {
          return actions.getText(session, elements.RESULTS_COUNT_LOCATOR, 'css')
